...
const buildEvent = (tournament: ITournament): string => {
  const eventMap = [
    ...
    ["DESCRIPTION", "https://start.gg/tournament/" + tournament.url],
    [
      "IMAGE",
      `IMAGE;VALUE=URI;DISPLAY=BADGE;FMTTYPE=image/png:${tournament.image}`,
    ],
    ["END", "VEVENT"],
  ];
...
const formatUTC = (d: Date): string => {
  ...
  const UTCDATE = `${d.getUTCFullYear()}${pad0(d.getUTCMonth() + 1)}${pad0(
    d.getUTCDate()
  )}`;
  const UTCTIME = `${pad0(d.getUTCHours())}${pad0(d.getUTCMinutes())}${pad0(
    d.getUTCSeconds()
  )}`;
