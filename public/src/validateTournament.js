...
export const createValidators = state => {
  return [
    ...
    newValidator("The smash.gg page does not contain a City", state.isOnline || empty(state.city)),
    newValidator("Tournament needs at least 1 event", () => state.events.length > 0),
    newValidator("At least one events needs an entry cap", () => state.nocap || state.events.some(e => e.cap && +e.cap > 0)),
