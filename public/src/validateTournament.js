...
const empty = (x) => () => x !== "";
const priceEmpty = (x) => () =>
  x.length === 1 && x[0].name === "" && x[0].amount === "";
...
export const createValidators = (state) => {
  return [
    ...
    newValidator(
      ...
      () => new Date(state.startDate) > Date.now()
    ),
    ...
    newValidator(
      "Tournament needs at least 1 event",
      () => state.events.length > 0
    ),
    newValidator(
      "At least one events needs an entry cap",
      () => state.nocap || state.events.some((e) => e.cap && +e.cap > 0)
    ),
    newValidator(
    ...
    newValidator(
      "A price needs a value",
      () =>
        priceEmpty(state.prices)() || state.prices.every((p) => p.amount !== "")
    ),
    newValidator(
      "A price needs a name",
      () =>
        priceEmpty(state.prices)() || state.prices.every((p) => p.name !== "")
    ),
  ];
};
